...
export default {
  ...
  methods: {
    ...
    async getAttendancesList() {
      this.loading = true
      // const qqq = await getAttendancesList({ ...this.page })
      // console.log(qqq)
      const { data } = await getAttendancesList({ ...this.page })
      this.list = data.data.rows // 当前记录
      this.page.total = data.data.total // 总条数
      ...
      const month = data.monthOfReport
