...
import { getPersonalDetailAPI, updatePersonalAPI, saveUserDetailByIdAPI, getUserDetailByIdAPI } from '@/api'
...
export default {
  ...
  methods: {
    ...
    async getPersonalDetail() {
      ...
      this.formData = data
      // 判断data 中是否有 staffPhoto数据，有的话就给mystaffPhoto组件
      if (data.staffPhoto) {
        this.$refs.myStaffPhoto.fileList = [{ url: data.staffPhoto, upload: true }]
      }
    },
    // 保存用户照片
    async savePersonal() {
      ...
      try {
        await updatePersonalAPI({ ...this.formData, id: this.userId, staffPhoto: fileList && fileList.length ? fileList[0].url : '' })
    ...
    async saveUser() {
      ...
      try {
        // 将两个信息进行合并
        await saveUserDetailByIdAPI({ ...this.userInfo, staffPhoto: fileList && fileList.length ? fileList[0].url : '' })
    ...
    async getUserDetailById() {
      ...
      this.userInfo = data
      // 判断userinfo中是否有用户的头像信息，如果有的话就直接信息放入 staffPhoto 组件的  list数组中
      if (data.staffPhoto) {
        this.$refs.staffPhoto.fileList = [{ url: data.staffPhoto, upload: true }]
      }
    }
  }
}
