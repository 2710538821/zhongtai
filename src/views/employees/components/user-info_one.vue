...
export default {
  ...
  methods: {
    async getUserDetailById() {
      ...
      // 有值就表示已经有了一个上传成功的图片了
      // 上传成功的图片内容加一个upload:true，就表示图片已经上传成功了
      // this.userInfo.data.staffPhoto.trim()是为了去除存储的图片为空的情况下
      if (this.userInfo.data.staffPhoto && this.userInfo.data.staffPhoto.trim()) {
        this.$refs.topStaff.fileList = [{ url: this.userInfo.data.staffPhoto, upload: true }]
      }
    },
    async getPersonalDetail() {
      this.formData = await getPersonalDetail(this.userId) // 获取员工数据
      if (this.formData.data.staffPhoto && this.formData.data.staffPhoto.trim()) {
        this.$refs.buttonStaff.fileList = [{ url: this.formData.data.staffPhoto, upload: true }]
      }
    },
    async saveUser() {
      // 调用父组件
      const fileList = this.$refs.topStaff.fileList
      if (fileList.some(item => !item.upload)) {
        this.$message.warning('此时图片还在上传中')
        return
      }
    ...
    async savePersonal() {
      // 调用父组件
      const fileList = this.$refs.buttonStaff.fileList
      if (fileList.some(item => !item.upload)) {
        this.$message.warning('此时图片还在上传中')
        return
      }
